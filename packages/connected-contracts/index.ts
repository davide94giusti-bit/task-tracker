...
export const InviteStatus = z.enum(['pending','accepted','expired','revoked','disabled']);
export const EmailAddress = z.string().trim().email().max(320).transform(value=>value.toLowerCase());
...
export const AccountDeletionRequest = z.object({confirmation:z.literal('DELETE MY ACCOUNT'),exportFirst:z.boolean().default(true)}).strict();
export const PasswordChange = z.object({password:z.string().min(12).max(128).regex(/[a-z]/).regex(/[A-Z]/).regex(/[0-9]/)}).strict();
export const IdentityState = z.object({status:z.enum(['active','invited','uninvited','disabled']),userId:Uuid,email:EmailAddress,workspaceId:Uuid.optional(),role:Role.optional(),platformAdmin:z.boolean(),requiresPasswordSetup:z.boolean().default(false)});
export const InviteRecord = z.object({id:Uuid,email:EmailAddress,status:InviteStatus,invitedAt:z.string(),expiresAt:z.string(),acceptedAt:z.string().nullable().optional(),acceptedUserId:Uuid.nullable().optional(),disabledAt:z.string().nullable().optional()});
export const UsageSummary = z.object({acceptedUsers:z.number().int().nonnegative(),pendingInvitations:z.number().int().nonnegative(),userLimit:z.number().int().positive(),records:z.record(z.string(),z.number().int().nonnegative()),approximateStorageBytes:z.number().int().nonnegative(),emailsSentToday:z.number().int().nonnegative(),emailsSentThisMonth:z.number().int().nonnegative(),failedNotifications:z.number().int().nonnegative(),pendingDeliveries:z.number().int().nonnegative(),oldestPendingDelivery:z.string().nullable(),lastCronAt:z.string().nullable(),lastExportAt:z.string().nullable()});
export const DeliveryCompletion = z.object({deliveryId:Uuid,status:z.enum(['delivered','retry','failed','quota_reached']),errorCode:z.string().max(100).optional(),nextAttemptAt:z.string().datetime({offset:true}).optional()}).strict();
export const ReminderClaim = z.object({limit:z.number().int().min(1).max(50).default(25)}).strict();
