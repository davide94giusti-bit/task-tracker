...
function PersonRecapDialog({ person, onClose }: { person: Person | null; onClose: () => void }) {
  const [details, setDetails] = useState<Person | null>(null), [tasks, setTasks] = useState<Task[]>([]), [shareUrl, setShareUrl] = useState(''), [loading, setLoading] = useState(false), [error, setError] = useState('');
  const updateLink = async (action: 'ensure' | 'regenerate' | 'revoke') => { if (!person) return; setError(''); try { const response = await api<{ active: boolean; token?: string }>('/people/share-link', { method: 'POST', body: { personId: person.id, action } }); setShareUrl(response.token ? `${location.origin}/shared-tasks?token=${encodeURIComponent(response.token)}` : ''); } catch (reason) { setError((reason as Error).message); } };
  useEffect(() => { if (!person) { setDetails(null); setTasks([]); setShareUrl(''); return; } let active = true; setLoading(true); setError(''); Promise.all([api<Person>(`/people/details?personId=${encodeURIComponent(person.id)}`), api<{ items: Task[] }>(`/tasks?view=all&responsiblePersonId=${encodeURIComponent(person.id)}&pageSize=100`), api<{ active: boolean; token?: string }>('/people/share-link', { method: 'POST', body: { personId: person.id, action: 'ensure' } })]).then(([contact, response, share]) => { if (active) { setDetails({ ...person, ...contact }); setTasks(sortRecapTasks(response.items)); setShareUrl(share.token ? `${location.origin}/shared-tasks?token=${encodeURIComponent(share.token)}` : ''); } }).catch(e => { if (active) setError((e as Error).message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [person]);
  const recap = details ? makeRecap(details, tasks, shareUrl) : null, whatsappPhone = details?.phone?.replace(/\D/g, '') || '';
  return <Dialog open={!!person} onClose={onClose} fullWidth maxWidth="sm"><DialogTitle>Send pending-task recap</DialogTitle><DialogContent><Stack spacing={2} mt={1}>{loading && <Stack alignItems="center"><CircularProgress size={28}/></Stack>}{error && <Alert severity="error">{error}</Alert>}{details && <><Typography><strong>{details.fullName}</strong> has {tasks.length} pending task{tasks.length === 1 ? '' : 's'}.</Typography>{tasks.length === 0 ? <Alert severity="info">There are no pending tasks to send.</Alert> : <Paper variant="outlined" sx={{ p: 2, maxHeight: 280, overflow: 'auto', whiteSpace: 'pre-wrap' }}>{recap?.body}</Paper>}<Alert severity="info">The master link is live and read-only. They can bookmark it and see current active tasks whenever they return. Regenerating or revoking it disables the old link.</Alert>{shareUrl ? <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap><Button size="small" variant="outlined" onClick={() => void navigator.clipboard.writeText(shareUrl)}>Copy master link</Button><Button size="small" onClick={() => void updateLink('regenerate')}>Regenerate</Button><Button size="small" color="error" onClick={() => void updateLink('revoke')}>Revoke</Button></Stack> : <Button variant="outlined" onClick={() => void updateLink('ensure')}>Create master link</Button>}{!details.phone && !details.email && <Alert severity="warning">Add a phone number or email address to this person before sending a recap.</Alert>}<Typography variant="caption" color="text.secondary">The app prepares the message. You review and send it from WhatsApp or your email app.</Typography></>}</Stack></DialogContent><DialogActions sx={{ flexWrap: 'wrap', gap: 1 }}><Button onClick={onClose}>Close</Button>{details && <Button onClick={() => downloadContact(details)}>Add to contacts</Button>}{details?.email && recap && tasks.length > 0 && <Button variant="outlined" onClick={() => { window.location.href = `mailto:${details.email}?subject=${encodeURIComponent(recap.subject)}&body=${encodeURIComponent(recap.body)}`; }}>Email recap</Button>}{whatsappPhone && recap && tasks.length > 0 && <Button variant="contained" onClick={() => window.open(`https://wa.me/${whatsappPhone}?text=${encodeURIComponent(`*${recap.subject}*\n\n${recap.body}`)}`, '_blank', 'noopener,noreferrer')}>WhatsApp recap</Button>}</DialogActions></Dialog>;
}
