...
import { Add, CalendarMonth, CheckCircle, CloudDone, Dashboard as DashboardIcon, DarkMode, Delete, Event, ExpandLess, ExpandMore, Folder, Groups, Inbox, InfoOutlined, LightMode, Logout, Menu, MenuBook, MoreHoriz, Notifications, People, Refresh, Settings, TaskAlt, Today } from '@mui/icons-material';
...
function PersonRecapDialog({ person, onClose }: { person: Person | null; onClose: () => void }) {
  const [details, setDetails] = useState<Person | null>(null), [tasks, setTasks] = useState<Task[]>([]), [share, setShare] = useState<ShareLinkState | null>(null), [loading, setLoading] = useState(false), [error, setError] = useState(''), [confirmation, setConfirmation] = useState('');
