...
import { ChecklistWorkspaceView, EnhancedDashboardView, EnhancedTasksView, NotificationBell, TaskDetailsDialog, TaskEditorDialog, WorkView } from './ConnectedEnhancements';
...
import { consumePendingInvitationPath, ProjectInvitationOnboarding } from './ProjectInvitationOnboarding';
import { PWA_INSTALL_RETURN_PATH_KEY, PwaInstallBanner, PwaInstallCard, PwaInstallInstructions, usePwaInstall } from './PwaInstallPrompt';
...
}) { return <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'flex-start' }} spacing={2} mb={3}>
<Box>
<Typography variant="h4">{title}</Typography>
<Typography color="text.secondary">{subtitle}</Typography>
</Box>{action && <Box sx={{ '& > .MuiButton-root': { width: { xs: '100%', sm: 'auto' }, whiteSpace: 'nowrap' } }}>{action}</Box>}</Stack>; }
...
type LinkedProject = Project & { shareId: string; token: string; verificationToken: string; allowChecklistUpdates: boolean; allowTaskCompletion: boolean; allowComments: boolean; allowCreateEditTasks: boolean; allowManageChecklistItems: boolean; expiresAt?: string | null };
...
function ProjectPeopleDialog({ project, onClose }: { project: Project | null; onClose: () => void }) {
  ...
  const [sendInvitation, setSendInvitation] = useState(false), [invitation, setInvitation] = useState({ access: 'read_only', allowComments: false, allowTaskCompletion: false, allowCreateTasks: false, allowManageChecklistItems: false, expiresAt: '', offerNotifications: true }), [createdInvitation, setCreatedInvitation] = useState<{ code: string; path: string; recipientEmail: string } | null>(null);
  ...
  const linkedPerson = items.find(item => item.personId === personId);
  const availablePeople = people.filter(person => !items.some(item => item.personId === person.id) || person.id === personId);
  const chooseMode = (nextCreating: boolean) => { setCreating(nextCreating); setPersonId(''); setAccess(emptyProjectPersonAccess); setSendInvitation(false); setCreatedInvitation(null); };
  const createInvitation = async (selectedPersonId: string) => {
    if (!project || !sendInvitation) return null;
    return api<{ code: string; path: string; recipientEmail: string }>('/people/project-invitation', { method: 'POST', body: { projectId: project.id, personId: selectedPersonId, ...invitation, expiresAt: invitation.expiresAt ? new Date(`${invitation.expiresAt}T23:59:59Z`).toISOString() : null } });
  };
  return <Dialog open={!!project} onClose={() => !busy && onClose()} fullScreen>
    <DialogTitle sx={{ pb: 1 }}>
      <Stack direction="row" spacing={1.5} alignItems="center"><Avatar sx={{ bgcolor: 'primary.main' }}><Groups/></Avatar><Box><Typography variant="h6">People in {project?.name}</Typography><Typography variant="body2" color="text.secondary">Link contacts and control what collaborators can see.</Typography></Box></Stack>
    </DialogTitle>
    <DialogContent dividers><Stack spacing={2.5}>
      <Paper variant="outlined" sx={{ p: 2, bgcolor: 'action.hover' }}><Stack direction="row" spacing={1.5} alignItems="flex-start"><InfoOutlined color="info"/><Box><Typography fontWeight={750}>What linking a person does</Typography><Typography variant="body2" color="text.secondary" mt={.5}>It associates an existing People record with this project. It does not assign tasks, create a login, or give that person access to Task Tracker. Removing the link keeps the person and their tasks.</Typography></Box></Stack></Paper>
      {error && <Alert severity="error">{error}</Alert>}{message && <Alert severity="success" onClose={() => setMessage('')}>{message}</Alert>}
      <Box>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.25}><Box><Typography variant="h6">Linked people</Typography><Typography variant="body2" color="text.secondary">Contact fields are private unless marked as shared.</Typography></Box><Chip size="small" label={items.length}/></Stack>
        {!items.length && <Paper variant="outlined" sx={{ p: 3, textAlign: 'center' }}><Groups color="disabled"/><Typography fontWeight={700} mt={1}>No people linked yet</Typography><Typography variant="body2" color="text.secondary">Choose an existing person or create a new contact below.</Typography></Paper>}
        <Stack spacing={1.25}>{items.map(item => {
          const shared = [item.sharePhone && 'Phone', item.shareEmail && 'Email', item.shareAddress && 'Address', item.shareNotes && 'Notes'].filter(Boolean) as string[];
          return <Paper key={item.personId} variant="outlined" sx={{ p: 2 }}><Stack spacing={1.5}>
            <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={1}><Box minWidth={0}><Typography fontWeight={800}>{item.fullName}</Typography><Typography variant="body2" color="text.secondary">{[item.role, item.company].filter(Boolean).join(' · ') || 'No role specified'}</Typography></Box>{item.supervisable && <Chip size="small" color="secondary" label="Supervisable"/>}</Stack>
            <Box><Typography variant="caption" color="text.secondary" fontWeight={700}>VISIBLE TO COLLABORATORS</Typography><Stack direction="row" flexWrap="wrap" useFlexGap gap={.75} mt={.5}>{shared.length ? shared.map(label => <Chip key={label} size="small" variant="outlined" label={label}/>) : <Typography variant="body2" color="text.secondary">No contact details shared</Typography>}</Stack></Box>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><Button variant="outlined" size="small" onClick={() => { setCreating(false); setPersonId(item.personId); setAccess({ sharePhone: item.sharePhone, shareEmail: item.shareEmail, shareAddress: item.shareAddress, shareNotes: item.shareNotes, supervisable: item.supervisable }); }}>Edit visibility</Button><Button variant="text" color="error" size="small" disabled={busy} onClick={async () => { if (!project) return; setBusy(true); try { await api('/people/project-people/remove', { method: 'POST', body: { projectId: project.id, personId: item.personId } }); setMessage(`${item.fullName} was unlinked from the project and remains in People.`); if (personId === item.personId) { setPersonId(''); setAccess(emptyProjectPersonAccess); } await load(); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); } }}>Remove from project</Button></Stack>
          </Stack></Paper>;
        })}</Stack>
      </Box>
      <Divider/>
      <Box><Typography variant="h6">Add someone to this project</Typography><Typography variant="body2" color="text.secondary" mb={1.5}>Use a person already in your directory or create a new contact.</Typography>
        <Paper variant="outlined" sx={{ p: 1 }}><Stack direction="row" spacing={1}><Button fullWidth variant={!creating ? 'contained' : 'text'} startIcon={<People/>} onClick={() => chooseMode(false)}>Existing person</Button><Button fullWidth variant={creating ? 'contained' : 'text'} startIcon={<Add/>} onClick={() => chooseMode(true)}>New person</Button></Stack></Paper>
      </Box>
      {creating ? <Stack spacing={1.5}><TextField autoFocus label="Full name" required value={form.fullName} onChange={e => setForm(v => ({ ...v, fullName: e.target.value }))}/><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}><TextField fullWidth label="Role or function" value={form.role} onChange={e => setForm(v => ({ ...v, role: e.target.value }))}/><TextField fullWidth label="Company" value={form.company} onChange={e => setForm(v => ({ ...v, company: e.target.value }))}/></Stack><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5}><TextField fullWidth label="Phone" value={form.phone} onChange={e => setForm(v => ({ ...v, phone: e.target.value }))}/><TextField fullWidth label="Email" type="email" value={form.email} onChange={e => setForm(v => ({ ...v, email: e.target.value }))}/></Stack><TextField label="Address" value={form.address} onChange={e => setForm(v => ({ ...v, address: e.target.value }))}/><TextField multiline minRows={2} label="Private notes" helperText="Notes remain private unless you explicitly share them below." value={form.notes} onChange={e => setForm(v => ({ ...v, notes: e.target.value }))}/></Stack> : <TextField select fullWidth label={linkedPerson ? 'Linked person' : 'Choose a person'} value={personId} helperText={linkedPerson ? 'You are editing this person’s project visibility.' : 'This links the directory record; it does not assign any tasks.'} onChange={e => { setPersonId(e.target.value); const current = items.find(item => item.personId === e.target.value); setAccess(current ? { sharePhone: current.sharePhone, shareEmail: current.shareEmail, shareAddress: current.shareAddress, shareNotes: current.shareNotes, supervisable: current.supervisable } : emptyProjectPersonAccess); }}><MenuItem value="">Select a person</MenuItem>{availablePeople.map(person => <MenuItem key={person.id} value={person.id}>{person.fullName}{person.role ? ` · ${person.role}` : ''}</MenuItem>)}</TextField>}
      <Paper variant="outlined" sx={{ p: 2 }}><Typography fontWeight={800}>Collaborator visibility</Typography><Typography variant="body2" color="text.secondary" mb={1}>These settings matter only when this project is shared with a collaborator.</Typography><Stack><FormControlLabel control={<Switch checked={access.sharePhone} onChange={e => setAccess(v => ({ ...v, sharePhone: e.target.checked }))}/>} label="Share phone number"/><FormControlLabel control={<Switch checked={access.shareEmail} onChange={e => setAccess(v => ({ ...v, shareEmail: e.target.checked }))}/>} label="Share email address"/><FormControlLabel control={<Switch checked={access.shareAddress} onChange={e => setAccess(v => ({ ...v, shareAddress: e.target.checked }))}/>} label="Share postal address"/><FormControlLabel control={<Switch checked={access.shareNotes} onChange={e => setAccess(v => ({ ...v, shareNotes: e.target.checked }))}/>} label="Share notes"/><Divider sx={{ my: 1 }}/><FormControlLabel control={<Switch checked={access.supervisable} onChange={e => setAccess(v => ({ ...v, supervisable: e.target.checked }))}/>} label="Allow authorised collaborators to update this person’s project work"/></Stack></Paper>
      <Paper variant="outlined" sx={{ p: 2 }}><FormControlLabel control={<Switch checked={sendInvitation} onChange={e => { setSendInvitation(e.target.checked); setCreatedInvitation(null); }}/>} label="Send an invitation to this project"/><Typography variant="body2" color="text.secondary">A project contact does not receive digital access unless you create an invitation.</Typography>{sendInvitation && <Stack spacing={1.5} mt={2}><TextField select label="Access" value={invitation.access} onChange={e => setInvitation(v => ({ ...v, access: e.target.value, ...(e.target.value === 'tasks' ? { allowTaskCompletion: true } : e.target.value === 'read_only' ? { allowTaskCompletion: false, allowCreateTasks: false, allowManageChecklistItems: false } : {}) }))}><MenuItem value="read_only">Read-only</MenuItem><MenuItem value="checklist">Can update assigned checklist items</MenuItem><MenuItem value="tasks">Can complete assigned tasks and checklist items</MenuItem></TextField><FormControlLabel control={<Switch checked={invitation.allowComments} onChange={e => setInvitation(v => ({ ...v, allowComments: e.target.checked }))}/>} label="Can comment"/><FormControlLabel control={<Switch checked={invitation.allowTaskCompletion} disabled={invitation.access === 'tasks' || invitation.access === 'read_only'} onChange={e => setInvitation(v => ({ ...v, allowTaskCompletion: e.target.checked }))}/>} label="Can complete assigned tasks"/><FormControlLabel control={<Switch checked={invitation.allowCreateTasks} disabled={invitation.access === 'read_only'} onChange={e => setInvitation(v => ({ ...v, allowCreateTasks: e.target.checked }))}/>} label="Can create and edit project tasks"/><FormControlLabel control={<Switch checked={invitation.allowManageChecklistItems} disabled={invitation.access === 'read_only'} onChange={e => setInvitation(v => ({ ...v, allowManageChecklistItems: e.target.checked }))}/>} label="Can manage checklist items"/><TextField type="date" label="Invitation expiry (optional)" value={invitation.expiresAt} onChange={e => setInvitation(v => ({ ...v, expiresAt: e.target.value }))} slotProps={{ inputLabel: { shrink: true } }}/><FormControlLabel control={<Switch checked={invitation.offerNotifications} onChange={e => setInvitation(v => ({ ...v, offerNotifications: e.target.checked }))}/>} label="Offer email and app notifications during onboarding"/></Stack>}{createdInvitation && <Alert severity="success" sx={{ mt: 2 }}>Invitation ready: <strong>{createdInvitation.code}</strong><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} mt={1}><Button size="small" onClick={() => void navigator.clipboard.writeText(`${location.origin}${createdInvitation.path}`)}>Copy invitation link</Button><Button size="small" href={`mailto:${createdInvitation.recipientEmail}?subject=${encodeURIComponent(`Task Tracker invitation · ${project?.name || ''}`)}&body=${encodeURIComponent(`${location.origin}${createdInvitation.path}`)}`}>Email invitation</Button></Stack></Alert>}</Paper>
    </Stack></DialogContent>
    <DialogActions sx={{ position: 'sticky', bottom: 0, bgcolor: 'background.paper', borderTop: 1, borderColor: 'divider', p: 2, pb: 'calc(16px + env(safe-area-inset-bottom))', gap: 1, flexDirection: { xs: 'column-reverse', sm: 'row' }, '& > .MuiButton-root': { m: '0 !important', width: { xs: '100%', sm: 'auto' }, minHeight: 44 } }}><Button disabled={busy} onClick={onClose}>Close</Button><Button variant="contained" disabled={busy || (creating ? !form.fullName.trim() : !personId)} onClick={async () => { setBusy(true); setError(''); try { let selectedPersonId = personId; if (creating) { const result = await api<{ personId: string }>('/people/project-people/create', { method: 'POST', body: { projectId: project?.id, ...form, fullName: form.fullName.trim(), email: form.email.trim().toLowerCase(), ...access } }); selectedPersonId = result.personId; } else await saveAssociation(personId); const invite = await createInvitation(selectedPersonId); if (invite) { setCreatedInvitation(invite); setMessage('Person added and secure project invitation created.'); } else { setMessage(creating ? 'New person created and linked to the project.' : linkedPerson ? 'Project visibility updated.' : 'Person linked to the project.'); setPersonId(''); setCreating(false); } setForm({ fullName: '', role: '', company: '', phone: '', email: '', address: '', notes: '' }); setAccess(emptyProjectPersonAccess); await load(); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); } }}>{busy ? <CircularProgress size={20}/> : sendInvitation ? 'Add and create invitation' : creating ? 'Create and add' : linkedPerson ? 'Save visibility' : 'Add to project'}</Button></DialogActions>
  </Dialog>;
}
function ProjectsView({ onOpen }: { onOpen: (project: Project) => void }) {
  ...
  return <>
    ...
    <Dialog open={open} onClose={() => !busy && setOpen(false)} fullWidth maxWidth="sm" fullScreen={mobile}>
      <DialogTitle sx={{ pb: 1 }}><Stack direction="row" spacing={1.5} alignItems="center"><Avatar sx={{ bgcolor: 'primary.main' }}><Folder/></Avatar><Box><Typography variant="h6">{editing ? 'Edit project' : 'Create a project'}</Typography><Typography variant="body2" color="text.secondary">Organise related tasks and track their combined progress.</Typography></Box></Stack></DialogTitle>
      <DialogContent dividers><Stack spacing={2.5}>
        {!editing && <Alert severity="info">Creating a project does not create tasks or add people automatically. You can connect both after saving.</Alert>}
        <TextField autoFocus required label="Project name" placeholder="For example: Chillinguito renovation" value={form.name} onChange={e => setForm(v => ({ ...v, name: e.target.value }))}/>
        <TextField multiline minRows={4} label="Description" helperText="Optional: describe the outcome, scope, or important context." value={form.description} onChange={e => setForm(v => ({ ...v, description: e.target.value }))}/>
        <Paper variant="outlined" sx={{ p: 2 }}><Stack direction="row" spacing={2} alignItems="center"><Box sx={{ width: 44, height: 44, flex: '0 0 auto', borderRadius: 1.5, bgcolor: form.color, border: 1, borderColor: 'divider' }}/><TextField fullWidth label="Project colour" type="color" value={form.color} onChange={e => setForm(v => ({ ...v, color: e.target.value }))} slotProps={{ inputLabel: { shrink: true } }}/></Stack><Typography variant="caption" color="text.secondary">Used on project cards and visual progress indicators.</Typography></Paper>
      </Stack></DialogContent>
      <DialogActions sx={{ p: 2, gap: 1, flexDirection: { xs: 'column-reverse', sm: 'row' }, '& > .MuiButton-root': { m: '0 !important', width: { xs: '100%', sm: 'auto' } } }}><Button disabled={busy} onClick={() => setOpen(false)}>Cancel</Button><Button variant="contained" disabled={busy || !form.name.trim()} onClick={async () => { setBusy(true); setError(''); try { await api('/projects/save', { method: 'POST', body: { ...form, name: form.name.trim(), ...(editing ? { id: editing.id, expectedVersion: editing.version } : {}) } }); setOpen(false); setMessage(editing ? 'Project updated.' : 'Project created.'); await load(); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); } }}>{busy ? <CircularProgress size={20}/> : editing ? 'Save changes' : 'Create project'}</Button></DialogActions>
    </Dialog>
...
function PersonRecapDialog({ person, onClose }: { person: Person | null; onClose: () => void }) {
  ...
  const [access, setAccess] = useState({ scopeMode: 'assigned' as 'assigned' | 'project', projectId: '', allowChecklistUpdates: false, allowTaskCompletion: false, allowComments: false, allowViewProjectContacts: false, allowViewContactAssignments: false, allowSuperviseContactChecklists: false, allowCompleteContactTasks: false, allowManageProjectContacts: false, allowCreateEditTasks: false, allowManageChecklistItems: false, expiresAt: '' });
  ...
  return <Dialog open={!!person} onClose={onClose} fullWidth maxWidth="sm">
    ...
    <DialogContent><Stack spacing={2} mt={1}>
      ...
      {details && <><Typography><strong>{details.fullName}</strong> has {tasks.length} pending task{tasks.length === 1 ? '' : 's'}.</Typography>
        ...
        <Alert severity="info">Choose exactly what this person may see and update. Assigned-work controls apply to their own tasks; project-wide authoring controls appear only when one entire project is shared. Every change requires verified access and is recorded.</Alert>
        {share?.active && <Accordion variant="outlined" disableGutters sx={{ '&:before': { display: 'none' } }}><AccordionSummary expandIcon={<ExpandMore />}><Box><Typography fontWeight={700}>Collaboration access</Typography><Typography variant="body2" color="text.secondary">Permissions and expiry · select to expand</Typography></Box></AccordionSummary><AccordionDetails><Stack spacing={1.5}>
          ...
          <Paper variant="outlined" sx={{ p: 1.5 }}><Typography fontWeight={700}>Assigned task permissions</Typography><Typography variant="body2" color="text.secondary" mb={.5}>These apply to tasks assigned to this person.</Typography><Stack>
            <FormControlLabel control={<Switch checked={access.allowChecklistUpdates} onChange={e => setAccess(v => ({ ...v, allowChecklistUpdates: e.target.checked }))}/>} label="Can tick checklist items on assigned tasks"/>
            ...
            <FormControlLabel control={<Switch checked={access.allowComments} onChange={e => setAccess(v => ({ ...v, allowComments: e.target.checked }))}/>} label="Can comment on assigned tasks"/>
          </Stack></Paper>
          {access.scopeMode === 'project' && <Paper variant="outlined" sx={{ p: 1.5 }}><Typography fontWeight={700}>Project work permissions</Typography><Typography variant="body2" color="text.secondary" mb={.5}>These apply only inside the selected shared project. They do not allow moving or deleting tasks.</Typography><Stack>
            <FormControlLabel control={<Switch checked={access.allowCreateEditTasks} onChange={e => setAccess(v => ({ ...v, allowCreateEditTasks: e.target.checked }))}/>} label="Can create and edit tasks"/>
            <FormControlLabel control={<Switch checked={access.allowManageChecklistItems} onChange={e => setAccess(v => ({ ...v, allowManageChecklistItems: e.target.checked }))}/>} label="Can manage checklist items"/>
          </Stack></Paper>}
...
<Alert severity="info">Task and checklist reminders support push/email. Checklist items due today also receive an idempotent in-app notification; overdue automation and daily summaries remain follow-up work.</Alert>
</Stack>}
	</CardContent>
	</Card>
<Card>
...
type IdentityState = { displayName?: string | null; email?: string | null; platformAdmin: boolean };
export default function App() { const [logged, setLogged] = useState(!!session.get()), [identity, setIdentity] = useState<IdentityState | null>(null), [accountAnchor, setAccountAnchor] = useState<HTMLElement | null>(null), [authMode,setAuthMode]=useState<'invite'|'recovery'|null>(null), [view, setView] = useState<View>((new URLSearchParams(location.search).get('view') as View) || 'dashboard'), [dark, setDark] = useState(matchMedia('(prefers-color-scheme: dark)').matches), [drawer, setDrawer] = useState(false), [desktopNav, setDesktopNav] = useState(true), [collapsedGroups, setCollapsedGroups] = useState<Record<string, boolean>>({}), [offline, setOffline] = useState(!navigator.onLine), [task, setTask] = useState<Task | null>(null), [deepLinkError, setDeepLinkError] = useState(''), [editingTask, setEditingTask] = useState<Task | null>(null), [newDate, setNewDate] = useState<string | null>(null), [refreshToken, setRefreshToken] = useState(0), [filter, setFilter] = useState<{
    title: string;
    q: Record<string, unknown>;
    kind?: 'task' | 'checklist';
    scope?: string;
} | null>(null);
const mobile = useMediaQuery('(max-width:800px)'); useEffect(()=>{void consumeAuthLink().then(mode=>{if(mode){setLogged(true);setAuthMode(mode)}})},[]); useEffect(()=>{if(logged)void api<IdentityState>('/access/state').then(setIdentity).catch(()=>setIdentity(null));else setIdentity(null)},[logged]); useEffect(() => { const params = new URLSearchParams(location.search), linkedTaskId = params.get('task'); if (params.get('source') === 'notification' && linkedTaskId && /^[0-9a-f-]{36}$/i.test(linkedTaskId)) localStorage.setItem(PWA_INSTALL_RETURN_PATH_KEY, `${location.pathname}${location.search}`); }, []); useEffect(() => { if (!logged) return; const params = new URLSearchParams(location.search), linkedTaskId = params.get('task'), finish = () => { localStorage.removeItem(PWA_INSTALL_RETURN_PATH_KEY); params.delete('task'); params.delete('source'); params.set('view', 'tasks'); history.replaceState(null, '', `?${params.toString()}`); }; if (!linkedTaskId || !/^[0-9a-f-]{36}$/i.test(linkedTaskId)) return; let active = true; setDeepLinkError(''); void api<{ task: Task }>(`/tasks/details?taskId=${encodeURIComponent(linkedTaskId)}`).then(details => { if (!active) return; setFilter(null); setView('tasks'); setTask(details.task); finish(); }).catch(error => { if (!active) return; finish(); setDeepLinkError((error as Error).message || 'The linked task could not be opened.'); }); return () => { active = false; }; }, [logged]); useEffect(() => { if (!logged) return; const params = new URLSearchParams(location.search), token = params.get('claimProject'); if (!token) return; void api('/people/claim-linked-project', { method: 'POST', body: { token } }).then(() => { sessionStorage.setItem('linked-project-claim-message', 'Project linked to your Task Tracker account.'); setView('projects'); history.replaceState(null, '', '?view=projects'); }).catch(error => { sessionStorage.setItem('linked-project-claim-message', (error as Error).message); setView('projects'); history.replaceState(null, '', '?view=projects'); }); }, [logged]); useEffect(() => { const online = () => { setOffline(false); void flushQueue(); };
...
const platformAdmin = Boolean(identity?.platformAdmin), accountName = identity?.displayName?.trim() || identity?.email?.split('@')[0] || 'Account', accountInitials = accountName.split(/\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase() || 'U';
const signOutHere = () => { setAccountAnchor(null); session.set(null); setLogged(false); };
const body = filter ? filter.kind === 'checklist' ? <ChecklistWorkspaceView title={filter.title} initialScope={filter.scope || 'open'} onOpen={openTask} refreshToken={refreshToken}/> : <EnhancedTasksView view="tasks" query={filter.q} title={filter.title} onOpen={openTask} onNew={() => openNewTask()} refreshToken={refreshToken}/> : view === 'dashboard' ? <EnhancedDashboardView openFilter={(title, q) => setFilter({ title, q, kind: 'task' })} openChecklistFilter={(title, scope) => setFilter({ title: `${title} checklist items`, q: {}, kind: 'checklist', scope })} openTask={openTask} refreshToken={refreshToken}/> : view === 'tasks' ? <WorkView onOpen={openTask} onNew={() => openNewTask()} refreshToken={refreshToken}/> : ['today', 'upcoming', 'completed', 'trash'].includes(view) ? <EnhancedTasksView view={view} onOpen={openTask} onNew={() => openNewTask()} refreshToken={refreshToken}/> : view === 'projects' ? <ProjectsView onOpen={project => setFilter({ title: project.name, q: { projectId: project.id } })}/> : view === 'people' ? <PeopleView onOpen={person => setFilter({ title: person.fullName, q: { responsiblePersonId: person.id } })}/> : view === 'dependencies' ? <DependencyLoadView onOpenTasks={(title, q) => setFilter({ title, q })}/> : view === 'calendar' ? <CalendarView onOpen={openTask} onNew={openNewTask} refreshToken={refreshToken}/> : view === 'settings' ? <SettingsView /> : view==='access'?(platformAdmin?<UsersAccessView/>:<Alert severity="info">Only the platform administrator can invite and manage application users.</Alert>):view==='security'?<AccountSecurity onProfileUpdated={profile => setIdentity(current => ({ platformAdmin: Boolean(current?.platformAdmin), ...current, ...profile }))}/>:view==='backup'?<BackupImportView/>:view==='diagnostics'?<DiagnosticsView/>:view==='manual'?<UserManualView/>:<Generic view={view}/>;
return <ThemeProvider theme={theme}><CssBaseline/>
...
<Chip label="Connected" size="small" color="success" sx={{ ml: 1, display: { xs: 'none', sm: 'inline-flex' } }}/>
<Box flex={1}/>{offline && <Chip color="warning" label="Offline"/>}<NotificationBell/><IconButton aria-label="Toggle color theme" onClick={() => setDark(!dark)}>{dark ? <LightMode /> : <DarkMode />}</IconButton>
<Button color="inherit" aria-label={`Account menu for ${accountName}`} aria-haspopup="menu" aria-expanded={Boolean(accountAnchor)} onClick={event => setAccountAnchor(event.currentTarget)} sx={{ minWidth: 0, px: mobile ? 0.5 : 1, ml: 0.5, gap: 1 }}>
<Avatar sx={{ width: 34, height: 34, bgcolor: 'primary.main', color: 'primary.contrastText', fontSize: 14, fontWeight: 800 }}>{accountInitials}</Avatar>
{!mobile && <Typography fontWeight={700} noWrap sx={{ maxWidth: 180 }}>{accountName}</Typography>}
</Button>
<AccountMenu anchorEl={accountAnchor} open={Boolean(accountAnchor)} onClose={() => setAccountAnchor(null)} MenuListProps={{ 'aria-label': 'Account options' }}>
<Box sx={{ px: 2, py: 1, maxWidth: 280 }}><Typography fontWeight={800} noWrap>{accountName}</Typography>{identity?.email && <Typography variant="body2" color="text.secondary" noWrap>{identity.email}</Typography>}</Box>
<Divider/>
<MenuItem onClick={() => { setAccountAnchor(null); navigate('security'); }}><ListItemIcon><Security fontSize="small"/></ListItemIcon>Profile &amp; security</MenuItem>
<MenuItem onClick={signOutHere}><ListItemIcon><Logout fontSize="small"/></ListItemIcon>Sign out</MenuItem>
</AccountMenu>
</Toolbar>
...
<BottomNavigationAction value="tasks" label="Work" icon={<Inbox />}/>
...
<Snackbar open={Boolean(deepLinkError)} autoHideDuration={8000} onClose={() => setDeepLinkError('')} message={deepLinkError}/>
</Box>
</ThemeProvider>; }
