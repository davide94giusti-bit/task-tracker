...
function ProjectPeopleDialog({ project, onClose }: { project: Project | null; onClose: () => void }) {
  ...
  const chooseMode = (nextCreating: boolean) => { setCreating(nextCreating); setPersonId(''); setAccess(emptyProjectPersonAccess); };
  return <Dialog open={!!project} onClose={() => !busy && onClose()} fullWidth maxWidth="md" fullScreen={mobile}>
    <DialogTitle sx={{ pb: 1 }}>
    ...
    <DialogActions sx={{ p: 2, gap: 1, flexDirection: { xs: 'column-reverse', sm: 'row' }, '& > .MuiButton-root': { m: '0 !important', width: { xs: '100%', sm: 'auto' } } }}><Button disabled={busy} onClick={onClose}>Close</Button><Button variant="contained" disabled={busy || (creating ? !form.fullName.trim() : !personId)} onClick={async () => { setBusy(true); setError(''); try { if (creating) await api('/people/project-people/create', { method: 'POST', body: { projectId: project?.id, ...form, fullName: form.fullName.trim(), email: form.email.trim().toLowerCase(), ...access } }); else await saveAssociation(personId); setMessage(creating ? 'New person created and linked to the project.' : linkedPerson ? 'Project visibility updated.' : 'Person linked to the project.'); setPersonId(''); setForm({ fullName: '', role: '', company: '', phone: '', email: '', address: '', notes: '' }); setAccess(emptyProjectPersonAccess); setCreating(false); await load(); } catch (reason) { setError((reason as Error).message); } finally { setBusy(false); } }}>{busy ? <CircularProgress size={20}/> : creating ? 'Create and add' : linkedPerson ? 'Save visibility' : 'Add to project'}</Button></DialogActions>
  </Dialog>;
}
