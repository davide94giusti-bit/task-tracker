...
type ShareLinkState = { active: boolean; token?: string; createdAt?: string; lastSharedAt?: string | null; lastSharedChannel?: string | null; emailEnabled?: boolean; pushEnabled?: boolean };
function PersonRecapDialog({ person, onClose }: { person: Person | null; onClose: () => void }) {
  const [details, setDetails] = useState<Person | null>(null), [tasks, setTasks] = useState<Task[]>([]), [share, setShare] = useState<ShareLinkState | null>(null), [loading, setLoading] = useState(false), [error, setError] = useState('');
  const shareUrl = share?.token ? `${location.origin}/shared-tasks?token=${encodeURIComponent(share.token)}` : '';
  const requestLink = async (action: 'ensure' | 'regenerate' | 'revoke' | 'mark-shared', channel?: 'email' | 'whatsapp' | 'copy') => { if (!person) return null; setError(''); try { const response = await api<ShareLinkState>('/people/share-link', { method: 'POST', body: { personId: person.id, action, channel } }); setShare(response); return response; } catch (reason) { setError((reason as Error).message); return null; } };
  useEffect(() => { if (!person) { setDetails(null); setTasks([]); setShare(null); return; } let active = true; setLoading(true); setError(''); Promise.all([api<Person>(`/people/details?personId=${encodeURIComponent(person.id)}`), api<{ items: Task[] }>(`/tasks?view=all&responsiblePersonId=${encodeURIComponent(person.id)}&pageSize=100`), api<ShareLinkState>('/people/share-link', { method: 'POST', body: { personId: person.id, action: 'ensure' } })]).then(([contact, response, link]) => { if (active) { setDetails({ ...person, ...contact }); setTasks(sortRecapTasks(response.items)); setShare(link); } }).catch(e => { if (active) setError((e as Error).message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [person]);
  ...
  return <Dialog open={!!person} onClose={onClose} fullWidth maxWidth="sm">
    ...
    <DialogContent><Stack spacing={2} mt={1}>
      {loading && <Stack alignItems="center"><CircularProgress size={28}/></Stack>}{error && <Alert severity="error">{error}</Alert>}
      {details && <><Typography><strong>{details.fullName}</strong> has {tasks.length} pending task{tasks.length === 1 ? '' : 's'}.</Typography>
        {tasks.length === 0 ? <Alert severity="info">There are no pending tasks to send.</Alert> : <Paper variant="outlined" sx={{ p: 2, maxHeight: 240, overflow: 'auto', whiteSpace: 'pre-wrap' }}>{recap?.body}</Paper>}
        <Alert severity="info">The master link opens a live, read-only portal grouped by project. The recipient chooses whether to enable email or browser updates.</Alert>
        ...
        {shareUrl ? <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3,minmax(0,1fr))' }, gap: 1 }}><Button size="small" variant="outlined" onClick={async () => { await navigator.clipboard.writeText(shareUrl); noteShared('copy'); }}>Copy link</Button><Button size="small" onClick={() => void requestLink('regenerate')}>Regenerate</Button><Button size="small" color="error" onClick={() => void requestLink('revoke')}>Revoke</Button></Box> : <Button variant="outlined" onClick={() => void requestLink('ensure')}>Create master link</Button>}
    ...
    <DialogActions sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,minmax(0,1fr))', sm: 'repeat(4,minmax(0,1fr))' }, gap: 1, p: 2 }}>
