...
export function ChecklistWorkspaceView({ onOpen, refreshToken = 0, initialScope = 'open', title = 'All checklist', embedded = false }: { onOpen: (task: Task) => void; refreshToken?: number; initialScope?: string; title?: string; embedded?: boolean }) {
  const [data, setData] = useState<ChecklistWorkspaceResponse | null>(null), [projects, setProjects] = useState<Project[]>([]), [scope, setScope] = useState(initialScope), [projectId, setProjectId] = useState(''), [search, setSearch] = useState(''), [error, setError] = useState(''), [pending, setPending] = useState<string | null>(null);
  useEffect(() => setScope(initialScope), [initialScope]);
  useEffect(() => { void api<Project[]>('/projects').then(setProjects).catch(() => setProjects([])); }, [refreshToken]);
  const load = useCallback(async () => { setError(''); try { setData(await api<ChecklistWorkspaceResponse>(`/tasks/checklists?${new URLSearchParams({ scope, search, ...(projectId ? { projectId } : {}), today: localDateKey(), pageSize: '100' })}`)); } catch (reason) { setError((reason as Error).message); } }, [scope, search, projectId, refreshToken]);
  ...
  return <Stack spacing={2}>
    ...
    <Paper variant="outlined" sx={{ p: 1.25 }}><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'minmax(180px,240px) minmax(180px,240px) minmax(220px,1fr)' }, gap: 1 }}>
      ...
      <TextField select size="small" label="Project" value={projectId} onChange={event => setProjectId(event.target.value)}><MenuItem value="">All projects</MenuItem>{projects.map(project => <MenuItem key={project.id} value={project.id}>{project.name}</MenuItem>)}</TextField>
      <TextField size="small" label="Search text" value={search} onChange={event => setSearch(event.target.value)} />
    </Box></Paper>
    ...
    <Stack spacing={1}>{data?.items.map(item => <Card key={item.id} variant="outlined"><CardContent sx={{ p: { xs: 1.5, sm: 2 }, '&:last-child': { pb: { xs: 1.5, sm: 2 } } }}><Stack spacing={1.25}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" gap={1} flexWrap="wrap">
        <FormControlLabel sx={{ m: 0, minHeight: 44, '& .MuiFormControlLabel-label': { fontWeight: 700 } }} control={<Checkbox checked={item.completed} disabled={pending === item.id} onChange={() => void toggle(item)} inputProps={{ 'aria-label': `${item.completed ? 'Reopen' : 'Complete'} checklist item ${item.description}` }} sx={{ minWidth: 44, minHeight: 44 }}/>} label={item.completed ? 'Completed' : 'Complete'}/>
        <Stack direction="row" gap={.5} flexWrap="wrap"><Chip size="small" label={item.completed ? 'Completed' : item.dueDate && item.dueDate < localDateKey() ? 'Overdue' : item.dueDate === localDateKey() ? 'Due today' : 'Open'} color={item.completed ? 'success' : item.dueDate && item.dueDate < localDateKey() ? 'error' : 'default'}/><Chip size="small" variant="outlined" label={item.required ? 'Required' : 'Optional'}/>{item.taskBlocked && <Chip size="small" color="warning" label="Blocked parent"/>}</Stack>
      </Stack>
      <Box minWidth={0}><Typography fontWeight={800}>{item.description}</Typography><Typography variant="body2" mt={.25}><strong>Task:</strong> {item.taskTitle}</Typography><Typography variant="body2" color="text.secondary"><strong>Project:</strong> {item.projectName} · <strong>Responsible:</strong> {item.responsiblePersonName}</Typography><Typography variant="caption" color="text.secondary" display="block" mt={.5}>Checklist due {item.dueDate ? new Date(`${item.dueDate}T12:00:00`).toLocaleDateString() : 'No date'} · Parent {item.taskStatus.replaceAll('_', ' ')} · {item.taskPriority} priority{item.taskDueDate ? ` · task due ${new Date(`${item.taskDueDate}T12:00:00`).toLocaleDateString()}` : ''}</Typography></Box>
      <Button variant="outlined" startIcon={<OpenInNew/>} sx={{ minHeight: 44, alignSelf: { sm: 'flex-end' } }} onClick={() => onOpen(checklistTask(item))}>Open task</Button>
